// Katakana learning page functionality
document.addEventListener('DOMContentLoaded', function() {
    // Only initialize if we're on the katakana page
    if (window.location.pathname.includes('/katakana')) {
        initializeKatakanaPage();
    }
});

/**
 * Initialize the katakana learning page
 */
async function initializeKatakanaPage() {
    // Fetch katakana data
    const katakanaData = await window.japaneseApp.fetchData('katakana');
    if (!katakanaData) {
        document.getElementById('katakanaGrid').innerHTML = '<p class="error">Failed to load katakana data. Please try refreshing the page.</p>';
        return;
    }

    // Initialize tabs
    initializeTabs(katakanaData);

    // Load basic katakana by default
    loadCharacterGrid('basic', katakanaData);

    // Initialize stroke order controls
    initializeStrokeControls();

    // Initialize common words section with audio playback
    initializeCommonWords();
}

/**
 * Initialize tab switching functionality
 * @param {Object} data - Katakana data object
 */
 ...
 * @param {string} type - Character type (basic, dakuon, combinations, extended)
 * @param {Object} data - Katakana data object
 */
...
function showCharacterDetails(charData) {
    ...
    } else if (charData.type === 'extended') {
        content += `<p>Extended katakana character used for foreign sounds</p>`;
    }
 ...
 * Initialize common words section with audio playback
 */
function initializeCommonWords() {
    // Add click handlers for common words
    const wordCards = document.querySelectorAll('.word-card');

    wordCards.forEach(card => {
        card.addEventListener('click', function() {
            const japanese = this.querySelector('.japanese').textContent;
            window.japaneseApp.playAudio(japanese);
        });
    });
}
